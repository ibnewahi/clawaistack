    ...
    async function loadSettings() {
      try {
        ...
        if (user) {
          ...
          if (data) {
            setForm({
              workspaceName: data.workspace_name || 'ClawAI Stack HQ',
            ...
            setForm((prev) => ({ ...prev, adminEmail: user.email }));
          }
        }
      } catch (err) {
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="flex items-center gap-4">
                <button
                  type="submit"
                  disabled={saving}
                  className="flex items-center gap-2 rounded-lg bg-accent px-4 py-2 text-xs font-semibold text-background hover:bg-accent/90 transition-colors disabled:opacity-50"
                >
                  {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save Settings
                </button>
                {saved && <span className="text-xs text-emerald-400 font-medium">Settings saved to Supabase!</span>}
              </div>
            </form>
          )}
        </main>
